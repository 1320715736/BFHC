"""
Surrogate modeling and SHAP analysis for current zigzag trials.

The script is safe to run while Optuna/COMSOL continues in the background when
it is pointed at a frozen CSV snapshot:
...
# Response surfaces for the three most represented N_RUNS values.
...
for ax, y_vals, title in [
    (axes[0], y_p03, "lifeAvgP03sphere (W)"),
    (axes[1], y_life, "Lifetime (h)"),
]:
...
# Surrogate-constrained search. This is advisory only; current submission uses
# measured COMSOL trials, not surrogate predictions, as hard evidence.
...
print(
    f"  Trial #{trial_ids[best_feas]}: N={best_exp_x[0]:.0f}, "
    f"L_RUN={best_exp_x[1]:.1f} mm, z_first={best_exp_x[2]:.2f} mm, "
    f"AvgP03={best_exp_p03:.1f} W, life={best_exp_life:.2f} h"
)
print(
    f"  Gain vs cylinder feasible best: "
    f"{(best_exp_p03 / CYLINDER_OPT_FEASIBLE_AVG_P03_W - 1) * 100:+.1f}%"
)
print("\nSurrogate-advised next candidate")
print(
    f"  N={best_n}, L_RUN={L_opt:.1f} mm, z_first={z_opt:.2f} mm, "
    f"pred AvgP03={pred_p03_opt:.1f} W, pred life={pred_life_opt:.2f} h"
)
