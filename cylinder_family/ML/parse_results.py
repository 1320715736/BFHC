"""
Step 2: 解析 150-trial 贝叶斯优化结果，生成汇总报告 + 可视化
输出:
  figures/optimization_history.png   — 收敛曲线
  figures/parallel_coordinate.png    — 平行坐标图
  figures/pareto_front.png           — 功率-寿命 Pareto 前沿
  figures/scatter_r1_r3.png          — r1-r3 散点色图（AvgP03 着色）
  stdout: 完整汇总表 + baseline 对比
"""
...
# 硬约束: 寿命 >= 30% baseline
LIFE_FLOOR = bl["lifetimeH"] * 0.30 if bl else 15.0
...
for i in order[:20]:
    ...
    print(f"{trial_ids[i]:>4} {data['r1_mm'][i]:>5.2f} {data['r2_mm'][i]:>5.2f} "
          f"{data['r3_mm'][i]:>5.2f} {data['r4_mm'][i]:>5.2f} "
          f"{data['Vwork_V'][i]:>6.3f} {data['initialTmax_K'][i]:>7.1f} "
          f"{lifetime[i]:>7.1f} {avg_p03[i]:>7.1f} "
          f"{data['selfViewLoss_pct'][i]:>5.1f} {ok_mark:>3}")

# ── 5. Pareto 最优 (feasible only) ──────────────────────────
feas_idx = np.where(feasible)[0]
if len(feas_idx) > 0:
    feas_p03 = avg_p03[feas_idx]
    feas_life = lifetime[feas_idx]
    # Pareto: 功率更高 AND 寿命更长
    pareto_mask = np.zeros(len(feas_idx), dtype=bool)
    for i in range(len(feas_idx)):
        dominated = False
        for j in range(len(feas_idx)):
            if i == j: continue
            if feas_p03[j] >= feas_p03[i] and feas_life[j] >= feas_life[i] and \
               (feas_p03[j] > feas_p03[i] or feas_life[j] > feas_life[i]):
    ...
    for rank, pi in enumerate(p_order, 1):
        ...
        print(f"  Rank {rank}: Trial #{trial_ids[gi]}  "
              f"[{data['r1_mm'][gi]:.2f}, {data['r2_mm'][gi]:.2f}, "
              f"{data['r3_mm'][gi]:.2f}, {data['r4_mm'][gi]:.2f}] mm  "
              f"AvgP03={avg_p03[gi]:.1f}W ({imp:+.1f}%)  "
              f"Life={lifetime[gi]:.1f}h  SV={data['selfViewLoss_pct'][gi]:.1f}%")
...
if bl:
    ...
    rows_cmp = [
        ("Vwork (V)", "Vwork_V", ".4f"),
        ("初始 Tmax (K)", "initialTmax_K", ".1f"),
        ("寿命 (h)", "lifetimeH", ".1f"),
        ("初始 P03sphere (W)", "initialP03sphere_W", ".1f"),
        ("寿命均值 P03sphere (W)", "lifeAvgP03sphere_W", ".1f"),
        ("自遮挡损失 (%)", "selfViewLoss_pct", ".1f"),
        ("侵蚀步数", "erosionSteps", ".0f"),
    ]
    ...
    if best_feas_i is not None:
        ...
        print(f"\n★ 满足约束的最优提升: {imp:+.1f}%  "
              f"(Trial #{trial_ids[best_feas_i]}  "
              f"[{data['r1_mm'][best_feas_i]:.3f}, {data['r2_mm'][best_feas_i]:.3f}, "
              f"{data['r3_mm'][best_feas_i]:.3f}, {data['r4_mm'][best_feas_i]:.3f}] mm)")
...
ax.set_xlabel("Trial #"); ax.set_ylabel("lifeAvgP03sphere (W)")
...
# 7b. Pareto 前沿图 (功率 vs 寿命)
fig, ax = plt.subplots(figsize=(8, 6))
infeasible = ~feasible
ax.scatter(lifetime[infeasible], avg_p03[infeasible], s=15, alpha=0.3,
           c="gray", label=f"Infeasible (Life<{LIFE_FLOOR:.0f}h)")
ax.scatter(lifetime[feasible], avg_p03[feasible], s=20, alpha=0.6,
           c="steelblue", label="Feasible")
if len(feas_idx) > 0 and pareto_mask.any():
    pg = pareto_global
    p_sort = np.argsort(lifetime[pg])
    ax.plot(lifetime[pg][p_sort], avg_p03[pg][p_sort], "r-o", markersize=6,
            label="Pareto front", zorder=5)
if bl:
    ax.scatter([bl["lifetimeH"]], [bl["lifeAvgP03sphere_W"]], s=100,
               marker="*", c="gold", edgecolors="black", zorder=10, label="Baseline")
ax.axvline(LIFE_FLOOR, color="red", linestyle=":", alpha=0.5, label=f"Life floor ({LIFE_FLOOR:.0f}h)")
ax.set_xlabel("Lifetime (h)"); ax.set_ylabel("lifeAvgP03sphere (W)")
...
colors = plt.cm.RdYlGn(np.linspace(0, 1, 30))  # green=best
...
# simplified: single axis version
plt.close(fig)
