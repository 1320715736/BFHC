"""
Status report for the cylinder rematch C1 search.

This script is read-only. It checks the rematch CSV/SQLite outputs and prints a
...
def print_csv_status():
    ...
    ok_rows = [
        row for row in rows
        if row.get("status") == "OK" and finite_float(row.get("objectiveScore")) is not None
    ]
    if ok_rows:
