"""D1 radiation metric-v2 validation and control reruns for cylinders."""
...
CASES = {
    "B2": {
        "case_id": "D1_B2_official_cylinder",
        "role": "official_baseline_control",
        "radii_mm": [2.5] * 8,
        "smoke_voltage_V": 1.168,
    },
    "trial68": {
        "case_id": "D1_cylinder_trial68",
        ...
        "smoke_voltage_V": 1.171875,
    },
}

FIELDS = [
    "case_id", "family", "role", "runMode", "timestamp",
    "radii_mm", "physicsVersion", "metricVersion", "geometryVersion",
    "radiationEscapeMethod", "spectralSplit_um", "thermalAmbient_K",
    "scoreAmbientTarget_K", "gitCommit", "gitDirty", "pythonVersion",
    "mphVersion", "Vwork_V", "initialTmax_K", "Tmin_K", "Tmean_K",
    "U_pct", "maxErosionTmax_K", "lifetimeH", "initialP03gross_W",
    ...
    "erosionSteps", "erosionSolveRetries", "voltagePolicy",
    "voltageObjective", "runnerStatus",
    "status", "failure", "elapsed_sec",
]
...
def stationary_result(runner, radii_m, voltage):
    runner._r0 = max(radii_m)
    runner._initial_radii = list(radii_m)
    runner._fail_radii = [
        radius * (1.0 - runner.failure_fraction) for radius in radii_m
    ]
    runner._init_model(radii_m)
    result = runner._solve_prepared(radii_m, voltage)
    loss = result.get("selfViewLoss_pct")
    return {
...
def main():
    ...
    parser.add_argument("--case", choices=["B2", "trial68", "all"],
                        default="all")
    ...
    try:
        for key in requested:
            ...
            radii_m = [value * 1e-3 for value in case["radii_mm"]]
            started = time.time()
            base = {
                ...
                "radii_mm": json.dumps(case["radii_mm"]),
                "physicsVersion": "thermal_s2s_v2",
                "metricVersion": runner.metric_version,
                "geometryVersion": "cylinder_segmented_v1",
