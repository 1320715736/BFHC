...
ZIGZAG_JAVA_REF = {'Vwork_V': 100.0, 'initialTmax_K': 3209.3, 'lifetimeH': 7.1277, 'initialP03sphere_W': 3711.29, 'initialPradSphere_W': 3881.58, 'lifeAvgP03sphere_W': 3539.04, 'lifeAvgPradSphere_W': 3703.73, 'selfViewLoss_pct': -3.77, 'erosionSteps': 11.0, 'elapsed_sec': 0.0}
SOLVED_STATUSES = {'OK', 'PRUNE_LIFETIME'}
FIELDS_FLOAT = ['N_RUNS', 'L_RUN_mm', 'z_first_mm', 'side_mm', 'Vwork_V', 'initialTmax_K', 'lifetimeH', 'initialP03sphere_W', 'initialPradSphere_W', 'lifeAvgP03sphere_W', 'lifeAvgPradSphere_W', 'selfViewLoss_pct', 'erosionSteps', 'elapsed_sec']
...
# Convert CSV rows into numeric arrays for ranking and plotting.
if not DATA.exists():
...
# Reference row is trial 0 when present; otherwise use the fixed Java baseline.
...
header = f'{'trial':>5} {'N':>3} {'L_mm':>7} {'zf_mm':>6} {'side':>7} {'V':>7} {'Tmax':>8} {'life_h':>8} {'AvgP03':>9} {'feas':>5}'
print(header)
print('-' * len(header))
for i in order[:20]:
    print(f"{trial_ids[i]:>5} {data['N_RUNS'][i]:>3.0f} {data['L_RUN_mm'][i]:>7.1f} {data['z_first_mm'][i]:>6.2f} {data['side_mm'][i]:>7.3f} {data['Vwork_V'][i]:>7.2f} {data['initialTmax_K'][i]:>8.1f} {lifetime[i]:>8.2f} {avg_p03[i]:>9.1f} {('Y' if feasible[i] else 'N'):>5}")
...
if len(feas_idx) > 0:
    ...
    for rank, local_i in enumerate(p_order, 1):
        ...
        print(f"  Rank {rank}: Trial #{trial_ids[gi]} | N={data['N_RUNS'][gi]:.0f}, L={data['L_RUN_mm'][gi]:.1f} mm, z_first={data['z_first_mm'][gi]:.2f} mm, side={data['side_mm'][gi]:.3f} mm | AvgP03={avg_p03[gi]:.1f} W, life={lifetime[gi]:.2f} h")
...
print(f'Best unconstrained zigzag row: Trial #{trial_ids[best_all_i]}, AvgP03={avg_p03[best_all_i]:.1f} W, life={lifetime[best_all_i]:.2f} h')
if best_feas_i is not None:
    print(f'Best feasible zigzag row: Trial #{trial_ids[best_feas_i]}, AvgP03={avg_p03[best_feas_i]:.1f} W, life={lifetime[best_feas_i]:.2f} h, gain vs cylinder optimized best={(avg_p03[best_feas_i] / CYLINDER_OPT_FEASIBLE_AVG_P03_W - 1) * 100:+.1f}%')
...
if len(pareto_global) > 0:
    ...
    ax.plot(lifetime[pareto_global][p_sort], avg_p03[pareto_global][p_sort], 'r-o', markersize=5, label='Feasible Pareto front', zorder=5)
...
# Parameter trends among high-performing trials.
