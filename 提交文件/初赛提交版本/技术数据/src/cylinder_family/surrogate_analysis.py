...
# Read solved trials and fit surrogate models on the independent variables.
...
LIFE_FLOOR = 0.3 * BASELINE_LIFETIME_H
...
rf_p03 = RandomForestRegressor(n_estimators=260, max_depth=10, min_samples_leaf=2, random_state=42, n_jobs=1)
rf_life = RandomForestRegressor(n_estimators=260, max_depth=10, min_samples_leaf=2, random_state=43, n_jobs=1)
...
# SHAP plots explain each radius contribution to power and lifetime.
...
# Response surface for r1-r3 while r4 remains volume-derived.
...
best_result = None
for seed in range(3):
    result = differential_evolution(constrained_objective, bounds, seed=seed, maxiter=120, popsize=10, tol=1e-06, polish=True, workers=1)
...
print(f'  Trial #{trial_ids[best_feas]}: [r1,r2,r3,r4]=[{best_exp_x[0]:.3f},{best_exp_x[1]:.3f},{best_exp_x[2]:.3f},{best_exp_r4:.3f}] mm, AvgP03={best_exp_p03:.1f} W, Lifetime={best_exp_life:.1f} h')
print('Surrogate-advised candidate')
print(f'  [r1,r2,r3,r4]=[{r1_opt:.3f},{r2_opt:.3f},{r3_opt:.3f},{r4_opt:.3f}] mm, pred AvgP03={pred_p03_opt:.1f} W, pred Lifetime={pred_life_opt:.1f} h')
